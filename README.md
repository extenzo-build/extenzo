...
**Summary:** CLI loads `exo.config`, resolves manifest (from config or `srcDir` / `srcDir/manifest`), discovers and resolves entries, then builds an Rsbuild config with **rsbuild-plugin-extension-entry** (entries + HTML), your **plugins** (e.g. Vue/React), and **rsbuild-plugin-extension-manifest** (writes `manifest.json` after build). In **dev**, **rsbuild-plugin-extension-hmr** starts a WebSocket server and opens the browser; each rebuild triggers a reload. In **build**, the output is written and optionally zipped.
