...
features:
  - title: 开发与生产一致
    ...
    icon: 📦
  - title: 零配置起步
    details: 脚手架生成项目或安装 extenzo 后，在根目录添加 exo.config.ts 即可开始开发；支持 Vanilla / Vue / React。
    icon: 🚀
  - title: 双浏览器支持
    details: 同一套代码，通过 manifest 的 chromium / firefox 分支与 -l chrome | edge | brave | vivaldi | opera | santa | firefox 分别构建与开发。
    icon: 🌐
  - title: 开发体验
...
- **一条命令**：`extenzo dev` / `extenzo build`，支持 `-l chrome|edge|brave|vivaldi|opera|santa|firefox` 指定目标浏览器。
- **约定优于配置**：在项目根或 `appDir` 下按约定放置 `background`、`content`、`popup`、`options`、`sidepanel`、`devtools` 等入口，或使用 `entry` 自定义。
- **插件化**：内置 plugin-entry、plugin-extension、plugin-extension-hmr；用户通过 `plugins: [vue()]` 或 `plugins: [react()]` 接入 Vue/React。
