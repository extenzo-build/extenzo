...
在 `exo.config.ts` 的 **plugins** 中接入 `@extenzo/rsbuild-plugin-vue` 即可启用 Vue 3 支持（SFC、Vue 运行时等）。
...
- **@extenzo/rsbuild-plugin-vue**：为 Vue 单文件组件、Vue 运行时等提供 Rsbuild 支持，内部使用 `@rsbuild/plugin-vue`、`@rsbuild/plugin-vue-jsx` 等。
