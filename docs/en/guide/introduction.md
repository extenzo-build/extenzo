...
- **Simple config**: One `ext.config.ts` (or `ext.config.js`) at the root for manifest, entry, plugins, and Rsbuild.
- **Dual browser**: Use manifest `chromium` / `firefox` branches and CLI `-b chrome` | `-b firefox` for output and debugging.
...
|------------|-------------|
| **Manifest** | Object config, path config, or omit to auto-load `manifest.json`, `manifest.chromium.json`, `manifest.firefox.json` from `srcDir` / `srcDir/manifest`, merged per browser. |
| **Entry discovery** | Default discovery of `background`, `content`, `popup`, `options`, `sidepanel`, `devtools` from root or `srcDir`; override with `entry`. |
| **Plugins** | Built-in plugin-entry (entry + HTML), plugin-extension (write manifest.json), plugin-hmr (dev HMR); add framework via `plugins: [vue()]`, `plugins: [react()]`. |
