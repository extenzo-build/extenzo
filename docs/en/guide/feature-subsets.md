...
| **srcDir** | Source directory, default project root; baseDir for entry and root for manifest auto-load. | [srcDir](/config/src-dir) |
...
| **plugins** | Rsbuild plugins array: built-in plugin-entry, plugin-extension, plugin-hmr; user adds vue(), react(). | [plugins](/config/plugins) |
...
- **extenzo dev**: Uses manifest, entry, plugins, launch, rsbuildConfig; HMR and browser launch depend on plugin-hmr and launch.
