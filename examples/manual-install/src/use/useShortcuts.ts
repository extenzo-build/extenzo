import { IMove, IScale, ActionType } from "src/types/type.d";
...
export const shortcutsMap = {
    ...
    // Player controls
    'Player': { 
        title: getMessage('tab_player'), 
        key: 'player', 
        trigger: ({ VideoRoll }: any) => {
            window.open(browser.runtime.getURL("player/player.html"), '_blank');
        }, 
        shortcuts: { key: '', code: [] } 
    },
    'Picture in Picture': { 
        title: getMessage('options_shortcut_pip'), 
        key: 'pictureInPicture', 
        handler: (data: boolean) => { return !data }, 
        shortcuts: { key: '', code: [] } 
    },
    
    // Summarizer
    'Summarizer': { 
        title: getMessage('video_summarizer'), 
        key: 'summarizer', 
        trigger: ({ VideoRoll }: any) => {
            sendRuntimeMessage(VideoRoll.rollConfig.tabId, {
                type: ActionType.OPEN_SIDE_PANEL,
                tabId: VideoRoll.rollConfig.tabId
            });
        }, 
        shortcuts: { key: '', code: [] } 
    },
    ...
    // VR controls
    'VR Mode': { 
        title: getMessage('options_shortcut_vr'), 
        key: 'vr', 
        handler: (data: any) => { 
            data.on = !data.on; 
            return data; 
    ...
    // 'Separation': { 
    //     title: getMessage('video_separate'), 
    //     key: 'advancedPictureInPicture', 
    //     handler: (data: any) => { 
    //         data.on = !data.on; 
    //         return data; 
    //     }, 
