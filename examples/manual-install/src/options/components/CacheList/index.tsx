...
import { showConfirmDialog } from "vant/es/dialog/index.mjs";
    ...
    const loadList = () => {
      ...
      browser.storage.sync.get().then((res) => {
        Object.keys(res).forEach((key) => {
          const data = res[key];
          if (
            key.startsWith("video-roll") &&
            !key.startsWith("video-roll-disabled") &&
            "deg" in data
          ) {
            list.value.push({
              url: data.url,
    ...
    const clear = () => {
      showConfirmDialog({
        ...
        browser.storage.sync.remove(
          list.value.map((item: any) => `video-roll-${item.url}`)
        );
        loadList();
      });
    };
    ...
    return () => (
      <div class="options-general">
        <div class="options-inside-header">
          <van-button type="primary" size="small" onClick={clear}>
            {browser.i18n.getMessage("tips_clearAll")}
          </van-button>
        </div>
