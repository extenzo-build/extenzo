...
import render from "app/options/utils/render";
  ...
  setup(props) {
      ...
      browser.storage.sync.get("generalConfig").then((res) => {
        ...
        if (data) {
          const array2Map = new Map(
            data.map((item: any) => [item.key, item.config])
          );
          ...
          config.value.forEach((item1) => {
            ...
            if (matchingConfig2) {
              const configMap2 = new Map(
                matchingConfig2.map((configItem: any) => [
                  configItem.key,
                  configItem.value,
                ])
              );
    ...
    return () => (
      <div class="options-general">
        <van-form submit="onSubmit">{render(config.value, onChange)}</van-form>
      </div>
    );
  },
});
