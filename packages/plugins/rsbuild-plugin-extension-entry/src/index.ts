...
export function buildJsChunkFilenameFn(
  ...
  return (pathData) => {
    ...
    if (name === SHARED_VENDOR_CHUNK_NAME) return "static/js/shared-vendor.js";
  ...
  return {
    ...
    setup(api: RsbuildPluginAPI) {
      api.modifyRsbuildConfig((config) => {
        ...
        // Use chunkSplit.override for sharedVendor so popup/options don't duplicate React
        config.performance = config.performance ?? {};
        config.performance.chunkSplit = config.performance.chunkSplit ?? {};
        const chunkSplit = config.performance.chunkSplit as Record<string, unknown>;
