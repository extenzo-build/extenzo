# AI usage (@extenzo/rsbuild-plugin-extension-hmr)
...
- Injected by **CLI pipeline** in dev; users set browser path via exo.config `launch.chrome` / `launch.firefox` or env (BROWSER_CHROME / BROWSER_FIREFOX)
...
- This plugin only handles “launch browser” and “HMR”; build is done by Rsbuild and rsbuild-plugin-extension-entry/rsbuild-plugin-extension-manifest
