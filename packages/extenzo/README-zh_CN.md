...
- `packages/plugins/plugin-extension-hmr`：**内部**，dev 热更新与启动浏览器
...
框架内部默认运行 plugin-extension-entry、plugin-extension-manifest、plugin-extension-hmr；用户通过 `plugins: [vue()]` 等引入框架插件，通过 `rsbuildConfig` 覆盖 Rsbuild。
